...
describe ("demoblaze", () => {
  ...
  test("user can filter categories", async() => {
    // the list of phones, laptops, and monitors the website uses
    ...
    /**
     * compare filtered elements to list of products
     * @param groupA - the hardcoded list of products
     * @param groupB - the filtered elements
     */
    let compareProducts = async(groupA, groupB) => {
      for (let i=0, j=0; i < groupA.length, j < groupB.length; i++, j++) {
        // expect filtered items to match the products
        expect(groupA[i]).toContain(groupB[j])
      }
    }
    // filter phones
    ...
    await compareProducts(phones, hardCodedFilteredBodyElements);
    // click laptops
    await demoblaze.click(By.xpath("//a[contains(text(),'Laptops')]"));
    // update hardCodedFilteredBodyElements for laptops
    hardCodedFilteredBodyElements = [
      await demoblaze.getText(By.xpath("//a[contains(text(),'Sony vaio i5')]")),
      await demoblaze.getText(By.xpath("//a[contains(text(),'Sony vaio i7')]")),
      await demoblaze.getText(By.xpath("//a[contains(text(),'MacBook air')]")),
      await demoblaze.getText(By.xpath("//a[contains(text(),'Dell i7 8gb')]")),
      await demoblaze.getText(By.xpath("//a[contains(text(),'2017 Dell 15.6 Inch')]")),
      await demoblaze.getText(By.xpath("//a[contains(text(),'MacBook Pro')]"))
    ];
    // expect filtered products to only be laptops
    await compareProducts(laptops, hardCodedFilteredBodyElements);
    await driver.sleep(500);

    // click monitors
    await demoblaze.click(By.xpath("//a[contains(text(),'Monitors')]"));
    // update hardCodedFilteredBodyElements for monitors
    hardCodedFilteredBodyElements = [
      await demoblaze.getText(By.xpath("//a[contains(text(),'Apple monitor 24')]")),
      await demoblaze.getText(By.xpath("//a[contains(text(),'ASUS Full HD')]"))
    ];
    await driver.sleep(500);
    // expect there to be only monitors
    await compareProducts(monitors, hardCodedFilteredBodyElements);
  });
